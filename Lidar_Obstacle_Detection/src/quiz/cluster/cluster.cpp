...
pcl::visualization::PCLVisualizer::Ptr initScene(Box window, int zoom)
{
	...
	viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 0 ,0, 0, "window");
  	return viewer;
}
...
int main ()
{
    ...
    for (int i=0; i<points.size(); i++) 
  	...
  	std::vector<int> nearby = tree->search({-6.0,6.5,1},4.0);
  	...
  	//
  	std::vector<std::vector<int>> clusters = euclideanCluster(points, tree, 7.0);
  	//
